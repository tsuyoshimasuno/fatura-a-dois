...
- source_spec: `bmad-output/implementation-artifacts/spec-8-2-consistencia-estrutural-e-telas-simples.md`
  summary: `text-[22.5px] font-bold` (tratamento de heading de seção dentro de `CardTitle`) está duplicado como literal Tailwind em ~11 pontos do código (`page.tsx`, `parcelas/page.tsx`, `lancamentos-view.tsx`, `cartoes/page.tsx`) em vez de centralizado numa classe/prop compartilhada.
  evidence: Achado real do Blind Hunter (review da Story 8.2) -- padrão pré-existente desde a Story 7.8 (Epic 7), esta story só adiciona mais 1 uso do mesmo padrão já aceito 10 vezes antes. Refactor para centralizar (`.card-section-title` ou prop `size` em `CardTitle`) é uma melhoria de manutenibilidade real, mas fora de escopo de uma story de polimento visual -- candidato a revisitar se mais telas ganharem heading de seção no futuro.
