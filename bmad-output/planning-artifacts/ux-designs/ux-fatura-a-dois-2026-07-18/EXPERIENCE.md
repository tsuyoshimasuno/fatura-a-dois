...
2. **Hierarquia tipográfica** — `[CORRIGIDO 2026-07-30, achado real da Story 8.2]` `{typography.section-title.fontSize}` (1.1rem/17.6px) NÃO é mais implementado como proposto: o Epic 7 (Story 7.8) já hardcoded 22.5px em 10 das 11 ocorrências de heading de seção via `CardTitle asChild className="text-[22.5px] font-bold"` (preservando o tamanho que já renderizava por default do navegador) — aplicar 1.1rem agora encolheria a única ocorrência restante (`.section-title` cru em `cartoes/page.tsx`) sem afetar as outras 10. Token corrigido para `22.5px` (valor real, ver `DESIGN.md` → Typography). `[IMPLEMENTADO 2026-07-30]` A última ocorrência crua (`cartoes/page.tsx`, "Cartões marcados como não sendo do casal") ganhou `text-[22.5px] font-bold` direto no `<h2>`, **sem** envolver a seção num `Card` -- achado real do review adversarial (1ª rodada, bad_spec repair): os itens dessa lista (`CartaoRejeitadoItem`) já são Cards individuais; um `Card` externo produziria "card dentro de card" (borda/sombra dobrada), o oposto de "mesmo tratamento do resto do produto". `.section-title` (CSS) removida, zero uso restante.
3. **Cards com mais respiro** — `{components.card.padding}` para `{spacing.4}` (1.75rem). `[CORRIGIDO 2026-07-30]` o token documentava `{spacing.2}`/`{spacing.3}` antes, mas isso descrevia a classe CSS artesanal `.card` (já morta desde o Épico 7); o `Card` vendorizado real em produção (`components/ui/card.tsx`) já usa `py-6`/`px-6` do Tailwind = 1.5rem — maior que os dois valores antigos, o que os tornaria uma redução. `{spacing.4}` é aumento real sobre o 1.5rem já implementado. Afeta `card`/`item-card`/`summary-card`/`card-highlight` uniformemente (mesmo token, via `py-*`/`px-*` do componente `Card`).
