...
**Deployment & ambientes:** um único ambiente de produção (Vercel + Supabase) — escala de 2 usuários não justifica staging formal ainda. Dev local via Supabase CLI (Postgres local em Docker) + `next dev`. Migrations via Drizzle Kit, aplicadas manualmente antes de cada deploy (sem pipeline de CI/CD formal nesta fase — ver Deferred). `[DESVIO: Point-in-Time Recovery do Supabase foi substituído por um job próprio de backup — o add-on de PITR custa US$100/mês (isolado do plano Pro de US$25/mês), desproporcional à escala de 2 usuários. Mecanismo adotado (Story 1.0): Vercel Cron dispara diariamente app/api/cron/backup (protegida por CRON_SECRET) → server/backup/dump.ts lista todas as tabelas public via information_schema, exporta cada uma para JSON e grava no bucket "backups" do Supabase Storage (service_role, bypassa RLS) com retenção de 30 dias. Satisfaz o mesmo NFR de durabilidade da PRD (§7): como o XLSX original é descartado por design (AD-5), os lançamentos estruturados são a única cópia dos dados, e esse dump é o mecanismo de recuperação caso sejam perdidos por bug ou falha. Revisitar se o volume de dados ou os requisitos de recovery point crescerem além do que um dump diário cobre.]`
